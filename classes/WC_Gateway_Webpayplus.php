...
/**
 * Description of WC_Gateway_Webpayplus
 *
 * @author ctala
 */
function init_webpayplus_class() {
    ...
    class WC_Gateway_Webpayplus extends WC_Payment_Gateway {
        ...
        var $politicas_devolucion;
        var $tiempos_envio;

        public function __construct() {
            $this->id = 'webpayplus';
            $iconAddress = WP_PLUGIN_URL . "/" . plugin_basename(dirname(__FILE__)) . '/assets/images/logo.png';
            log_me($iconAddress, "ICONO");
            ...
            $this->politicas_devolucion = $this->settings['politicas-devoluciones'];
            $this->despachos = $this->settings['despachos'];
        ...
        function init_form_fields() {
            ...
            $this->form_fields = array(
                ...
                'despachos' => array(
                    'title' => __('Despachos', 'woocommerce'),
                    'type' => 'textarea',
                    'description' => __('Mensaje que recibirán los clientes sobre los despachos al finalizar la compra.'),
                    'default' => __('Recuerda que tus productos serán enviados por correo a tu domicilio si es que seleccionaste este medio de despacho y deberían estar entre 2 y 3 días hábiles a tu hogar.'),
                ),
...
//                $TBK_MONTO = filter_input(INPUT_POST, "TBK_MONTO");
//
//                $validaOCMonto = $this->validarOrdenMonto($TBK_ORDEN_COMPRA, $TBK_MONTO);
//
//                if ($validaOCMonto) {
//                    $TBK_STATUS_ORDEN = $this->check_webpay_response();
//                } else {
//                    $TBK_STATUS_ORDEN = "failure";
//                }
        ...
        public function xt_compra() {
          ...
          if ($TBK_MONTO == $monto && $TBK_ORDEN_COMPRA == $ordenCompra && $acepta == true) {
            ...
            if ($acepta == true) {
                ...
                log_me("RESULTADO EXEC '$cmdline' ($retint) " . print_r($result, true), $sufijo);
        ...
        function check_webpay_response() {
            ...
            //Por defecto falla. Le debemos demostrar lo contrario.
            $status = "failure";
            ...
            if (isset($TBK_ID_SESION) && isset($TBK_ORDEN_COMPRA)) {
                ...
                try {
                    ...
                    if (!$order)
                        die("Orden que se intenta pagar no existe");
                    ...
                    if ($order->status !== 'completed') {
                         ...
                         * El archivo puede no existir por dos razones.
                         * La primera es que haya fallado el pago.
                         * La segunda es que estén intentando realizar una inyección.
                         * Para ambos casos rechazaremos.
                         */

                        if (!file_exists($myPath)) {
                            log_me("FALLO EN EL PAGO DE LA ORDEN DEBIDO A FALTA DE ARCHIVO", $SUFIJO);
                            $order->update_status('failed');
                            $order->add_order_note('Falló Archivo MAC');

                            //SobreEscribo el valor de status.
                            $status = "failure";
                            return $status;
                        } else {
                            log_me("Archivo Existente, se inicia validacióm", $SUFIJO);
                        ...
                        if ($status == 'success' && $TBK['TBK_ORDEN_COMPRA'][1] == $TBK_ORDEN_COMPRA) {
                            log_me("Montos y OC corresponden además de éxito", $SUFIJO);
                            /*
                             * VAlidación extra
                             * En caso de llegar acá corroboramos que los montos pagados
                             * Sean los mismos que los montos a pagar.
                             * De no ser así seteamos status failure y agregamos nota.
                             */
...
//
        ...
        /*
         * Funcion que valida que la OC sea existente y que el monto corresponda
         * al monto señalado en la Base De Datos
         */

        function validarOrdenMonto($orden_id, $monto) {
            $SUFIJO = "[VALORDER]";
            $order = new WC_Order($orden_id);
            if ($order) {
                //La orden existe
                $montoOrden = $order->order_total . "00";
                log_me("Monto a pagar recibido " . $monto, $SUFIJO);
                log_me("Monto a pagar del pedido " . $montoOrden, $SUFIJO);

                log_me("Comparando $monto con $montoOrden", $SUFIJO);
                if ($monto == $montoOrden) {
                    /*
                     * El monto a pagar corresponde.
                     */
                    log_me("Montos Corresponden", $SUFIJO);
                    return true;
                } else {
                    /*
                     * El monto a cancelar no corresponde
                     */
                    log_me("Montos NO Corresponden", $SUFIJO);
                    return false;
                }
            } else {
                //La orden no existe
                log_me("OC no existente");
                return false;
            }
        }
        ...
        public static function order_received($order_id = 0, $order_key = "") {
            ...
            echo "<header class='title woowpp_despachos_title'><h3>" . __('Despachos', 'woocommerce') . "</h3></header>";
            echo "<p class='woowpp_despachos'>";
            echo WC_Gateway_Webpayplus::webpay_get_option('despachos');
            echo '</p>';
            echo "<header class='title woowpp_devoluciones_title'><h3>" . __('Devoluciones', 'woocommerce') . "</h3></header>";
            echo "<p class='woowpp_devoluciones'>";
            echo WC_Gateway_Webpayplus::webpay_get_option('politicas-devoluciones');
            echo '</p>';
        }
        ...
        public static function webpay_status_valido($status = "", $order = null) {
            $status = trim($status);
